...
// === Floating Button to Open the Custom Skin Menu ===
let openCSLButton = document.createElement("button");
openCSLButton.innerText = "🎨 Skin Menu";
openCSLButton.style.position = "fixed";
openCSLButton.style.top = "20px";
openCSLButton.style.right = "20px";
openCSLButton.style.zIndex = "9999";
openCSLButton.style.padding = "10px 14px";
openCSLButton.style.backgroundColor = "#222";
openCSLButton.style.color = "#fff";
openCSLButton.style.border = "none";
openCSLButton.style.borderRadius = "5px";
openCSLButton.style.cursor = "pointer";
openCSLButton.style.boxShadow = "0 0 5px rgba(0,0,0,0.3)";

// Toggle visibility on click
openCSLButton.addEventListener("click", () => {
  if (!document.body.contains(option_group)) {
    document.body.appendChild(option_group);
    handleHighlight();
  } else {
    option_group.style.display =
      option_group.style.display === "none" ? "block" : "none";
  }
});

document.body.appendChild(openCSLButton);
